...
function Success(props) {
    ...
    const deleteCar = async (e, props) => {
        const { data, error } = await supabase.from("parking").update({"is_active": "false"}).eq("user_id", props.token.user.id).eq("is_active", "true");
        if(data) {
			...
			<div className="App">
				<header className="App-header">
					<Container>
						...
						{mapUrl && <iframe height="500px" width="500px" src={mapUrl}></iframe>}
					</Container>
				</header>
			</div>
		</>
